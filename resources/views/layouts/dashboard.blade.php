@php
    $role = auth()->check() ? auth()->user()->role->value : 'admin';

    if ($role === 'company') {
        $navItems = [
            ['route' => 'company.dashboard', 'icon' => 'bi-speedometer2', 'label' => 'Dashboard'],
            ['route' => 'company.job.create', 'icon' => 'bi-plus-circle', 'label' => 'Post Lowongan'],
            ['route' => 'company.profile', 'icon' => 'bi-building', 'label' => 'Profil Perusahaan'],
        ];
        $panelTitle = 'Panel Perusahaan';
    } else {
        $navItems = [
...
<head>
    ...
    <title>@yield('title', $panelTitle) — Magang Portal</title>
    ...
    <style>
        ...
        .nav-admin .nav-link.logout {
            color: var(--bs-danger);
        }

        .nav-admin .nav-link.logout i {
            color: var(--bs-danger);
        }
    ...
    <div class="admin-wrap">
        ...
        <div class="admin-main">
            <header class="admin-topbar">
                ...
                <div class="user">
                    @if(auth()->check() && auth()->user()->role->value !== 'admin')
                        <a href="{{ route('notification.list') }}" class="text-decoration-none d-flex align-items-center">
                            <div class="position-relative mt-1">
                                @php $unreadCount = auth()->user()->unreadNotifications->count(); @endphp
                                <i
                                    class="bi bi-bell-fill fs-5 {{ $unreadCount > 0 ? 'text-warning' : 'text-secondary' }}"></i>
                                @if($unreadCount > 0)
            ...
            <footer class="admin-footer">
                <span>© {{ date('Y') }} Magang Portal — {{ $panelTitle }}</span>

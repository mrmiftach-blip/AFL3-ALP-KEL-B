...
class JobController extends Controller
{
    ...
    {
        // Start query untuk model JobPosting beserta relasi companyProfile dan studyPrograms
        $query = JobPosting::query()->with(['companyProfile', 'studyPrograms']);
        ...
        if ($request->has('study_program') && $request->study_program != '') {
            $query->whereHas('studyPrograms', function ($q) use ($request) {
                $q->where('study_programs.id', $request->study_program);
            });
        }
    ...
    function single(string $id, Request $request)
    {
        ...
        $job = JobPosting::with(['companyProfile.user', 'studyPrograms'])->findOrFail($id);
